...
def main(input_filename: str, outdir: str = "reports"):
    ...
    if not os.path.exists(input_path):
        raise FileNotFoundError(
            ...
            f"Place '{input_filename}' in the project root:\n"
            f"{project_root}"
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Run the ML pipeline. Use: python -m src.run_pipeline train.csv"
    )
    parser.add_argument(
        "input",
        help="CSV filename located in the project root directory",
    )
    parser.add_argument(
        "--outdir",
        default="reports",
